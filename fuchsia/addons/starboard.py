...
class Star:
    ...
    def __repr__(self):
        return (
            ...
            "message_id={0.message_id} fetched={0.fetched}>".format(self)
        )

    async def edit(self, content: str):
        if self.fetched is None:
            self.fetched = await self.starboard_message.fetch()
        if self.fetched.flags.components_v2:
            view = ui.LayoutView.from_message(self.fetched)
            view.find_item(1).content = content  # type: ignore
            await self.fetched.edit(view=view)
        else:
            await self.starboard_message.edit(content=content)
    ...
    async def create_star(
        ...
        async with self.lock:
            ...
            if ref := message.reference:
                if ref.type == discord.MessageReferenceType.forward:
                    ...
                    container.add_item(
                        ...
                        ui.TextDisplay(
                            shorten(ref.resolved.content, 500) or "*no content*"
                        )
                    )
                    row.add_item(
            ...
            if ref:
                ...
                container.add_item(
                    ui.TextDisplay(
                        shorten(display_message.content, 1024) or "*no content*"
                    )
                )
                row.add_item(
                    ui.Button(url=message.jump_url, label="Go to message")
            ...
            if embeds := list(
                filter(
                    lambda e: e.type in ("rich", "poll_result"),
                    display_message.embeds,
                )
            ):
                container.add_item(ui.TextDisplay(f"**Embeds**"))
                container.add_item(
                    ui.TextDisplay(
                        "\n".join(
                            (
                ...
                *display_message.embeds,
                ...
                for attach in attachments[:10]:
                    if (
                        ...
                        gallery.add_item(
                            media=attach.url, spoiler=attach.is_spoiler()
                        )
...
class StarboardAddon(
    ...
    async def __ainit__(self):
        ...
        for guild_id in self.bot.configs.keys():
            ...
            self.starboards[guild_id] = await self.create_starboard(settings)
        self.ready = True
    ...
    def predicate(
        ...
        check_ignored = [  # Ensure the channel/message isn't ignored
            ...
            and payload.message_author_id in starboard.ignored,
        ]
    ...
    async def starboard_ignore(
        ...
        """
        Ignores a channel, message, or user
    ...
    async def starboard_unignore(
        ...
        """Unignores a channel, message, or user"""
        assert (
            interaction.guild
            and isinstance(interaction.channel, discord.abc.Messageable)
            and not isinstance(interaction.channel, discord.GroupChannel)
